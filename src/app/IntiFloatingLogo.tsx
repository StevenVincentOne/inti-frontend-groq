...
export const IntiFloatingLogo: React.FC<IntiFloatingLogoProps> = ({ 
  ...
  const handleMenuAction = (action: string) => {
    ...
    switch (action) {
      case 'uco_test': {
        // Build a real session identifier for /uco-test routing
        const urlParams = new URLSearchParams(window.location.search);
        // 1) Prefer explicit URL session params
        let sessionId = urlParams.get('session') || urlParams.get('sessionId');

        // 2) Try known local/session storage keys
        if (!sessionId) {
          sessionId =
            localStorage.getItem('intellipedia_session') ||
            sessionStorage.getItem('sessionId') ||
            null;
        }

        // 3) Try cookies (intellipedia_session, sessionId, or connect.sid)
        if (!sessionId) {
          const cookies = document.cookie.split(';').map(c => c.trim());
          const kv = (name: string) => cookies.find(c => c.startsWith(name + '='))?.split('=')[1];
          sessionId = kv('intellipedia_session') || kv('sessionId') || null;

          // Parse signed Express cookie if present (s:<id>.<sig> or URL-encoded)
          if (!sessionId) {
            const rawConnectSid = kv('connect.sid');
            if (rawConnectSid) {
              try {
                let parsed = decodeURIComponent(rawConnectSid);
                if (parsed.startsWith('s:')) parsed = parsed.slice(2);
                const dot = parsed.indexOf('.');
                if (dot !== -1) parsed = parsed.slice(0, dot);
        ...
        // 4) As a final fallback, check inti_auth for a stored sessionId (do NOT synthesize user_*)
        if (!sessionId) {
          try {
        ...
        // If still no real session, route to login flow instead of generating a fake value
        if (!sessionId) {
          handleLogin();
          break;
        }

        // Navigate to UCO test page with the resolved session param
        const target = `/uco-test?session=${encodeURIComponent(sessionId)}`;
      ...
      case 'profile': {
        const replitUrl = 'https://6d3f40b3-1e49-4b09-85e4-36ff422ee88b-00-psvr1owg24vj.janeway.replit.dev/creator';
        window.location.href = replitUrl;
        break;
      }
      case 'intellipedia':
        window.open('https://intellipedia.ai', '_blank');
        break;
  ...
  return (
    <>
      ...
      {showMenu && (
        <div className="fixed z-[1100] menu-fade-in" style={{ position: "fixed", bottom: "1.5rem", right: "4.5rem", zIndex: 1100 }}>
          <div className="bg-black/90 backdrop-blur-sm rounded-lg p-4 shadow-2xl border border-white/10" style={{ minWidth: '480px' }}>
            <div className="grid grid-cols-3 gap-4 mb-4">
              ...
              <div className="space-y-2">
                ...
                {user && (
                  <>
                    <div className="text-xs text-white/40 mt-3 px-2">Admin</div>
                    <button
                      onClick={() => handleMenuAction('uco_test')}
                      className="w-full text-left px-3 py-2 text-white hover:bg-white/10 rounded-md transition-colors flex items-center gap-2 border border-white/10"
                    >
              ...
              <div className="space-y-2">
                ...
                <button
                  onClick={() => handleMenuAction('intellipedia')}
                  className="w-full text-left px-3 py-2 text-white hover:bg-white/10 rounded-md transition-colors flex items-center gap-2"
                >
                  <span className="text-green-400">🌐</span>
                  Intellipedia
      ...
      <div className="fixed bottom-6 right-6 z-[1050]" style={{ position: "fixed", bottom: "1.5rem", right: "1.5rem", zIndex: 1050 }}>
        <button
